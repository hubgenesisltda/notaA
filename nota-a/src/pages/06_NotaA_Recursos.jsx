...
function Previsao(){
  ...
  const calcular=async()=>{setLoading(true);try{const d=await callAI({messages:[{role:"user",content:`Especialista ENEM. Notas atuais: lin:${notas.lin},hum:${notas.hum},nat:${notas.nat},mat:${notas.mat},red:${notas.red}. Horas extras/sem: lin:${horas.lin},hum:${horas.hum},nat:${horas.nat},mat:${horas.mat},red:${horas.red}. Meta:700. JSON:{"projecoes":{"lin":0,"hum":0,"nat":0,"mat":0,"red":0},"mediaAtual":0,"mediaProjetada":0,"probabilidadeMeta":0,"estrategia":"...","areaFoco":"...","prazo":"..."}`}],modulo:'previsao'});setRes(JSON.parse(d.content[0].text.replace(/```json|```/g,"").trim()));}catch{setRes({projecoes:{lin:680,hum:650,nat:590,mat:560,red:700},mediaAtual:566,mediaProjetada:636,probabilidadeMeta:72,estrategia:"Foque em Matemática e Natureza nas próximas 4 semanas. Mantenha a redação acima de 700.",areaFoco:"Matemática",prazo:"8 semanas"});}setLoading(false);};
  return(
    <div>
